...
const Header: React.FC<HeaderProps> = ({
  ...
  <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between gap-2">
    ...
    <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 flex-shrink-0">
      ...
      <h1 className="hidden sm:block text-lg sm:text-xl font-bold tracking-tight text-slate-900 dark:text-white truncate">
        Nöbet Takip
      </h1>
    </div>

    {/* Navigation */}
    <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1 flex-1 mx-2 min-w-0">
      <button
        onClick={onPrevMonth}
        className="p-2 hover:bg-white dark:hover:bg-slate-700 hover:shadow-sm rounded-md transition-all text-slate-600 dark:text-slate-300 flex-shrink-0"
        aria-label="Önceki Ay"
      >
        <ChevronLeft className="w-5 h-5" />
      </button>

      <span className="flex-1 text-center px-2 py-1 font-semibold text-slate-700 dark:text-slate-200 truncate">
        {monthName} {year}
      </span>
    ...
    <div className="flex items-center gap-2 flex-shrink-0">
      ...
      <button
        ...
        className="
          flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg font-medium transition-all
